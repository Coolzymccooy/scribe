...
const TeamWorkspaceView: React.FC<TeamWorkspaceViewProps> = ({
  ...
  const [teamMeetings, setTeamMeetings] = React.useState<any[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [tab, setTab] = React.useState<"library" | "members" | "launch">("library");
  ...
  // Persist orgId locally so it survives page reload
  const storedOrgId = React.useMemo(
    () => localStorage.getItem("scribe_org_id") || "",
    []
  );

  // Load org on mount if we have a stored orgId
  React.useEffect(() => {
    ...
    getOrg(storedOrgId)
      .then((o) => {
        if (o) {
          setOrg(o);
          return fetchOrgMeetings(o.id);
        }
        return [];
      })
      .then(setTeamMeetings)
      .catch(() => showToast("Could not load team data", "info"))
      .finally(() => setLoading(false));
  }, [storedOrgId]);

  const handleCreateOrg = async () => {
    if (!firebaseUid || !newOrgName.trim()) return;
    setLoading(true);
    try {
      const newOrg = await createOrg(firebaseUid, userEmail || firebaseUid, newOrgName.trim());
      localStorage.setItem("scribe_org_id", newOrg.id);
      setOrg(newOrg);
      showToast(`Org "${newOrg.name}" created!`);
    } catch {
      showToast("Could not create org", "info");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleJoinOrg = async () => {
    if (!firebaseUid || !inviteCodeInput.trim()) return;
    setLoading(true);
    try {
      // Look up invite code directly from Firestore — works even after server restarts
      const entry = await findOrgByInviteCode(inviteCodeInput.trim());
      if (!entry) { showToast("Invite code not found", "info"); setLoading(false); return; }
      const foundOrg = await getOrg(entry.orgId);
      if (!foundOrg) { showToast("Org not found in database", "info"); setLoading(false); return; }
      const member = {
        uid: firebaseUid,
      ...
      await joinOrg(entry.orgId, member);
      const updatedOrg = await getOrg(entry.orgId);
      localStorage.setItem("scribe_org_id", entry.orgId);
      setOrg(updatedOrg);
      setTeamMeetings(await fetchOrgMeetings(entry.orgId));
      showToast(`Joined "${foundOrg.name}"!`);
    } catch {
      showToast("Join failed", "info");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleShareMeeting = async (meeting: any) => {
    if (!org || !firebaseUid) return;
    try {
      await shareToOrg(org.id, firebaseUid, meeting);
      const updated = await fetchOrgMeetings(org.id);
      setTeamMeetings(updated);
      showToast(`"${meeting.title}" shared to team`);
    } catch {
      showToast("Share failed", "info");
    }
  };
  ...
  const handleCopyInviteLink = () => {
    ...
    navigator.clipboard.writeText(org.inviteCode).then(
      () => showToast(`Invite code copied: ${org.inviteCode}`),
      () => showToast(org.inviteCode, "info")
    );
  };
  ...
  const handleRefreshCode = async () => {
    if (!org) return;
    try {
      const code = await refreshInviteCode(org.id);
      setOrg({ ...org, inviteCode: code });
      showToast("Invite code refreshed");
    } catch {
  ...
  if (!firebaseUid) {
    return (
      <div className="max-w-2xl mx-auto mt-16 text-center space-y-4">
        ...
        <h2 className="text-2xl font-black">Sign in to use Team Workspace</h2>
        <p className="text-slate-500 text-sm">Team features require a ScribeAI account.</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-8">
        <header className="space-y-2">
          <h1 className="font-tech-display text-5xl md:text-7xl font-black tracking-tight uppercase">Team.</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">
            Shared recordings · Members · Smart Launch
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Join */}
          <div className="p-8 rounded-[2.5rem] bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 space-y-4 shadow-xl">
            <h3 className="text-xl font-black tracking-tight">Join a Workspace</h3>
            <p className="text-sm text-slate-500">Enter the invite code your team admin shared.</p>
            <input value={inviteCodeInput} onChange={e => setInviteCodeInput(e.target.value.toUpperCase())}
              ...
              className="w-full rounded-2xl px-4 py-3 bg-slate-100 dark:bg-white/5 border border-white/10 text-sm font-mono outline-none placeholder-slate-400 dark:text-white" />
            <button onClick={handleJoinOrg} disabled={loading || !inviteCodeInput.trim()}
              className="w-full py-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-white font-black text-[10px] uppercase tracking-widest disabled:opacity-50 transition-all">
  ...
  // ── Inside an org ────────────────────────────────────────────────────────
  const myRole = org.members.find((m: any) => m.uid === firebaseUid)?.role ?? "viewer";
    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {myRole === "owner" && (
            <button onClick={handleRefreshCode}
              className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-white/5 border border-white/10 text-slate-500 text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">
              ↻ New Code
            </button>
          )}
        </div>
      ...
      {tab === "library" && (
        <div className="space-y-4">
          {/* Share a personal meeting */}
          <div className="p-5 rounded-2xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 space-y-3">
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500">Share a Recording</p>
            <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto pr-1">
              ...
              {meetings.filter(m => m.transcript.length > 0).length === 0 && (
                <p className="text-sm text-slate-500">No processed recordings yet.</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {teamMeetings.length === 0 && (
              <div className="text-center py-12 text-slate-500 text-sm">No shared recordings yet — share one above!</div>
            )}
            {teamMeetings.map((m: any) => (
              <div key={m.id} className="p-5 rounded-2xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-sm font-black truncate">{m.title}</p>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">Shared by {m.sharedBy === firebaseUid ? "you" : m.sharedBy.slice(0, 8) + "…"} · {m.transcript?.length ?? 0} segments</p>
                </div>
                {(myRole === "owner" || m.sharedBy === firebaseUid) && (
                  <button onClick={() => removeFromOrg(org.id, m.id).then(() => setTeamMeetings(tm => tm.filter(x => x.id !== m.id)))}
                    className="text-[9px] font-black uppercase tracking-widest text-slate-500 hover:text-red-400 px-3 py-1.5 rounded-lg border border-red-500/20 hover:border-red-500/50 transition-all shrink-0">
                    Remove
                  </button>
                )}
              </div>
      ...
      {tab === "members" && (
        <div className="space-y-3">
          {org.members.map((m: any, i: number) => (
            <div key={i} className="p-5 rounded-2xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 flex items-center justify-between">
              <div>
        ...
        <div className="space-y-4">
          <div className="p-8 rounded-[2.5rem] bg-gradient-to-br from-indigo-600/20 to-purple-600/10 border border-indigo-500/20 space-y-5 shadow-xl">
            ...
            <ol className="space-y-4">
              {[
                { n: "1", icon: "📅", title: "Join your meeting", body: "Click your Google Meet / Zoom / Teams link to open the meeting in your browser." },
                { n: "2", icon: "🎧", title: "Enable Share Meeting Audio", body: "Return to ScribeAI → Live Studio → click 'Share Meeting Audio'. Choose the meeting tab and tick 'Share audio'." },
            ...
            <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20">
              <p className="text-[10px] font-black uppercase tracking-widest text-amber-400 mb-1">💡 Pro tip</p>
              <p className="text-xs text-slate-400">Works with <strong className="text-white">any browser tab meeting</strong> — Google Meet, Zoom web client, Teams web, Webex. No plugins needed. Chrome + Edge give the best system audio quality.</p>
            </div>
          </div>
        </div>
      )}
